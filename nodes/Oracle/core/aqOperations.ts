...
export class AQOperations {
  ...
  async enqueueBatch(
    ...
    try {
      ...
      const executionTime = Date.now() - startTime;
      this.log('info', `✅ Batch concluído: ${successCount}/${messages.length} sucessos (${executionTime}ms)`);
  ...
  async dequeueMultiple(
    ...
    try {
      ...
      const executionTime = Date.now() - startTime;
      this.log('info', `✅ Dequeue múltiplo concluído: ${successCount} mensagens (${executionTime}ms)`);
  ...
  private buildEnqueuePLSQL(payloadType: AQPayloadType, options: Required<AQEnqueueOptions>): string {
    return `
      DECLARE
  ...
  private buildEnqueueBinds(queueName: string, message: AQMessage, payloadType: AQPayloadType): any {
    const binds: any = {
      queue_name: queueName,
