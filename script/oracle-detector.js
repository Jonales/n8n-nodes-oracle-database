  ...
  async detectSystemOracleClient() {
    ...
    try {
      ...
      if (platform === 'win32') {
        ...
        command = 'which sqlplus 2>/dev/null || find /opt/oracle -name "libclntsh.so*" 2>/dev/null | head -1';
      }
...
async function getOracleClientConfig(options = {}) {
  ...
  try {
    ...
    // Configura LD_LIBRARY_PATH se necessário
    if (
      detection.available &&
      detection.libDir &&
      ...
      if (platform === 'linux' || platform === 'darwin') {
        ...
        const newPath = currentPath ? `${detection.libDir}:${currentPath}` : detection.libDir;
